(function () {
  ...
  window.open = async function (url, isABSENTURL = false) {
    try {
      // Check if it's an HTTP(S) URL
      if (url.startsWith("http://") || url.startsWith("https://") && isABSENTURL === false) {
      ...
      } else if (isABSENTURL === true) {
        // Handle ABSENT URLs directly
        return window.parent.tabs.createTab(url);
      }

      // Default behavior for non-HTTP(S) and non-ABSENT URLs
      return null;
    } catch (error) {
